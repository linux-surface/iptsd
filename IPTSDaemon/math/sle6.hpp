...
#ifndef IPTSD_MATH_SLE6_HPP
#define IPTSD_MATH_SLE6_HPP
...
void lu_solve(Mat6<T> const &lu, Vec6<index_t> const &p, Vec6<T> const &b, Vec6<T> &x)
{
	...
	y[4] = pb[4] - lu[{4, 0}] * y[0] - lu[{4, 1}] * y[1] - lu[{4, 2}] * y[2] -
	       lu[{4, 3}] * y[3];
	y[5] = pb[5] - lu[{5, 0}] * y[0] - lu[{5, 1}] * y[1] - lu[{5, 2}] * y[2] -
	       lu[{5, 3}] * y[3] - lu[{5, 4}] * y[4];
	...
	x[0] = y[0] - lu[{0, 5}] * x[5] - lu[{0, 4}] * x[4] - lu[{0, 3}] * x[3] -
	       lu[{0, 2}] * x[2] - lu[{0, 1}] * x[1];
	x[0] /= lu[{0, 0}];
}
 ...
 */
template <class T> auto ge_solve(Mat6<T> a, Vec6<T> b, Vec6<T> &x, T eps = num<T>::eps) -> bool
{
	// TODO: optimize/unroll?
	...
	x[0] = b[0] - a[{0, 5}] * x[5] - a[{0, 4}] * x[4] - a[{0, 3}] * x[3] - a[{0, 2}] * x[2] -
	       a[{0, 1}] * x[1];
	x[0] /= a[{0, 0}];
