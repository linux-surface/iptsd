...
UinputDevice::UinputDevice(void)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("Failed to open uinput device");
...
void UinputDevice::set_evbit(i32 ev)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("UI_SET_EVBIT failed");
}
...
void UinputDevice::set_keybit(i32 key)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("UI_SET_KEYBIT failed");
}
...
void UinputDevice::set_propbit(i32 prop)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("UI_SET_PROPBIT failed");
}
...
void UinputDevice::set_absinfo(u16 code, i32 min, i32 max, i32 res)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("UI_ABS_SETUP failed");
}
...
void UinputDevice::create(void)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("UI_DEV_SETUP failed");

	ret = ioctl(this->fd, UI_DEV_CREATE, nullptr);
	if (ret == -1)
		throw iptsd::utils::cerror("UI_DEV_CREATE failed");
}
...
void UinputDevice::emit(u16 type, u16 key, i32 value)
{
	...
	if (ret == -1)
		throw iptsd::utils::cerror("Failed to write input event");
}
