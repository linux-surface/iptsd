...
public:
	...
	bool predict(const clock::duration prediction_target)
	{
		...
		if (m_has_fixed_report_rate)
			m_expected_prediction_sample_size = gsl::narrow<isize>(std::ceil(pt / rr));
		...
		auto prediction_target_in_samples =
			gsl::narrow<isize>(std::ceil(pt / rr * confidence_factor));
